...
* **丰富的地图元素**: 不仅仅是墙壁，还有需要钥匙才能通过的门锁，增加了策略维度。以后还会有更多元素加入！
...
3. **环境**
    * **墙壁**: 无法通过的障碍。
    * **门**: 可以自由通过。
    * **锁 (带数字的墙)**: 必须拥有**等于或大于**锁上所示数字的钥匙数量，才能通过。通过时不会消耗钥匙。
...
2. **触发追捕**
    * 鬼的AI核心是**直线视野**。如果一只鬼和你处于同一行或同一列，且中间没有任何墙壁或锁阻挡，它就会“看到”你。
    * 一旦被看到，鬼会立刻将你锁定为目标，并沿着最短路径向你冲来！
...
* **放置/删除**:
  ...
  * **其他元素 (门、锁、钥匙、鬼、终点等)**: 单击画布上的有效位置即可放置。再次单击已放置的元素可以将其移除。
  * **橡皮擦**: 可以擦除任何你放置的墙壁或实体。
